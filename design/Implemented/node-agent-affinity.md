...
flowchart TD
    ...
    F --> I{Check StorageClass in loadAffinityPerStorageClass}
    H --> I
    ...
    K --> M{Check final StorageClass in loadAffinityPerStorageClass}
    L --> I
    
    I -->|StorageClass found| N[Apply loadAffinityPerStorageClass<br/>for this StorageClass<br/>🎯 HIGHEST PRIORITY]
    I -->|StorageClass not found| O{Check loadAffinity configuration}
    
    M -->|StorageClass found| N
    M -->|StorageClass not found| O
    
    O -->|Multiple loadAffinity entries| P[Apply all loadAffinity rules<br/>as OR conditions<br/>📋 MEDIUM PRIORITY]
    O -->|Single loadAffinity entry| Q[Apply single loadAffinity rule<br/>📋 MEDIUM PRIORITY]
    ...
    P --> V[Validate node-agent availability<br/>⚠️ Ensure node-agent pods exist on target nodes]
    Q --> V
    T --> V
    U --> V
