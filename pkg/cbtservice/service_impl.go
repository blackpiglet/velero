...
func (e *emitterImpl) SnapshotMetadataIteratorRecord(recordNumber int, metadata iterator.IteratorMetadata) error {
	for _, b := range metadata.BlockMetadata {
		...
		if b.ByteOffset < 0 || b.SizeBytes < 0 {
			e.logger.Errorf("Invalid CBT metadata: offset: %d, size %d", b.ByteOffset, b.SizeBytes)
			return fmt.Errorf("invalid CBT metadata: offset: %d, size %d", b.ByteOffset, b.SizeBytes)
		}

		e.logger.Debugf("recording metadata for record number %d, offset: %d, size: %d",
		...
		if err := e.recordCallBack([]Range{{Offset: uint64(b.ByteOffset), Length: uint64(b.SizeBytes)}}); err != nil {
			e.logger.WithError(err).Error("error recording CBT metadata")
			return err
		}
	}
